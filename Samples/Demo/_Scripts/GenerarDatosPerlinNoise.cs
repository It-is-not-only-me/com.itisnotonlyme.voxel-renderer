...
namespace ItIsNotOnlyMe.VoxelRenderer
{
    public class GenerarDatosPerlinNoise : GenerarDatos
    {
        ...
        private void Awake()
        {
            ...
            for (int i = 0, contador = 0; i < _datosPorEje.x; i++)
                for (int j = 0; j < _datosPorEje.y; j++)
                    for (int k = 0; k < _datosPorEje.z; k++, contador++)
                    {
                        _datos[contador].Color = new Vector3(0, 0, 1);
                        _datos[contador].Transparente = (j < _datosPorEje.y / 2) ? 0 : 1;
                    }
        }
